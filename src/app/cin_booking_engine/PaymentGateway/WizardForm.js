...
const WizardSidebar = ({ isVisible, onClose, status }) => {
  const { selectedStartDate, setSelectedStartDate, selectedEndDate, setSelectedEndDate, totalPrice, setTotalPrice, selectedRooms, selectedRoom, setSelectedRoom,
    setUserDetails, selectedRoomDetails, setSelectedRoomDetails, currentStep, setCurrentStep, addonList } = useBookingEngineContext();
  ...
  const [steps, setsteps] = useState([
    ...
    ...(addonList?.length > 0 ? [{ title: "AddOns", icon: <FontAwesomeIcon icon={faHouse} /> }] : []),
  ...
  useEffect(() => {
    ...
    const resetTimer = () => {
      ...
      timeout = setTimeout(() => {
        const confirmRefresh = window.confirm("Session expired due to inactivity.");
        if (confirmRefresh) {
  ...
  const goNext = () => {
    ...
    if (addonList?.length == 0 && currentStep < steps.length - 1 && currentStep == 0) {
        ...
        <div className="nav-sidebar" style={{ overflowY: "auto" }}>
          <div className="nav flex-column nav-pills me-0">
            {steps.map((step, index) => (
              <button key={index} className={`nav-link ${index === currentStep ? "active" : "active1"}`} onClick={() => index != 4 && setCurrentStep(index)}>
                <span>{step.icon}</span>
                {step.title}
              </button>
            ))}
