...
} from "app/cin_context/BookingEngineContext";
...
export default function BookNowForm({
  ...
  const hotelOptions = [
    ...
    {
      value: "lotus-court-noida",
      label: "The Hoften Lotus Court, Noida",
      slug: "lotus-court-noida",
      url: "",
    },
    {
      value: "sterling-plaza-banglore",
      label: "The Hoften Sterling Plaza, Bangalore",
      slug: "sterling-plaza-banglore",
      url: "",
    },
    {
      value: "xenious-amritsar",
      label: "Xenious Amritsar",
      slug: "xenious-amritsar",
      url: "",
    },
    {
      value: "leaf-resort-goa",
      label: "Xenious Cashew Leaf Resort, Goa",
      slug: "leaf-resort-goa",
      url: "",
    },
  ];
  ...
  const [cinBookingEngine, setCINBookingEngine] = useState(false);
  ...
  useEffect(() => {
    ...
    // }
      ...
      setSelectedHotel(hotelOptions.find((opt) => opt.value === "51757"));
    ...
    } else if (lowerPath.includes("lotus-court-noida")) {
      setSelectedHotel(
        hotelOptions.find((opt) => opt.value === "lotus-court-noida")
      );
    } else if (lowerPath.includes("sterling-plaza-banglore")) {
      setSelectedHotel(
        hotelOptions.find((opt) => opt.value === "sterling-plaza-banglore")
      );
    } else if (lowerPath.includes("xenious-amritsar")) {
      setSelectedHotel(
        hotelOptions.find((opt) => opt.value === "xenious-amritsar")
      );
    } else if (lowerPath.includes("leaf-resort-goa")) {
      setSelectedHotel(
        hotelOptions.find((opt) => opt.value === "leaf-resort-goa")
      );
    }
  }, [pathname]);
  ...
  const handleCheckAvailability = () => {
    ...
    if (
      ...
      selectedHotel.value === "lotus-court-noida" ||
      selectedHotel.value === "sterling-plaza-banglore" ||
      selectedHotel.value === "xenious-amritsar" ||
      selectedHotel.value === "leaf-resort-goa"
    ) {
    ...
    // }
    else if (selectedHotel.value === "51757") {
      // bookingUrl += `&checkIn=${checkIn}&checkOut=${checkOut}&clientWidth=1280&noofrooms=${countroom}`;
      // formRows.forEach((row, index) => {
      //   bookingUrl += `&adult${index}=${row.count1}&child${index}=${row.count2}`;
      // });
      // bookingUrl += `&gsId=981NJ8TQ49ro3Z7RTrbLaPgGZCWk8ihVQvzMylYvu085aOhvZIjS2TE3NTc=&RoomID=184902,184903,185018,185019,184904,185017,185949`;
      bookingUrl = "";
      setCINBookingEngine(true);
    }
    if (selectedHotel.value != "51757") {
